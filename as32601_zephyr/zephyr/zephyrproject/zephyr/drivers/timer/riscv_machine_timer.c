...
volatile uint32_t as32x601_timer_isr_count;
volatile uint32_t as32x601_sys_clock_announce_count;
...
static void set_mtimecmp(uint64_t time)
{
	...
	*(volatile uint64_t *)get_hart_mtimecmp() = time;
	...
	uint32_t lo = (uint32_t)time;
	uint32_t hi = (uint32_t)(time >> 32);

	/*
	 * AS32X601 CLINT follows its HAL programming model where the timer
	 * comparator is updated low-word first, then high-word. The generic
	 * RISC-V sequence (write high=0xffffffff, then low, then final high)
	 * does not latch on this SoC and leaves the comparator at 0.
	 */
	r[0] = lo;
	r[1] = hi;
#endif
...
static void timer_isr(const void *arg)
{
	...
	as32x601_sys_clock_announce_count += dticks;
