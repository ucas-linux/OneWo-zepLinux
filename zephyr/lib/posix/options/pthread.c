...
{
	int effective_policy = (policy == SCHED_OTHER) ? SCHED_RR : policy;
	int priority;

	if (effective_policy == SCHED_FIFO) {
		__ASSERT_NO_MSG(z_prio < 0);
...
int zephyr_to_posix_priority(int z_prio, int *policy)
{
	*policy = (z_prio < 0) ? SCHED_FIFO : SCHED_RR;
	...
	return zephyr_prio_to_posix_priority(z_prio, *policy);
}
		   ...
		   void *arg)
{
	...
	if (t->attr.inheritsched == PTHREAD_INHERIT_SCHED) {
		int pol = zephyr_policy_to_posix_policy(
			z_thread_sched_policy_get(k_current_get()));

		t->attr.priority = zephyr_prio_to_posix_priority(
			k_thread_priority_get(k_current_get()), pol);
	...
	k_thread_create(
		...
		posix_to_zephyr_priority(t->attr.priority, t->attr.schedpolicy), 0, K_FOREVER);

	K_SPINLOCK(&_sched_spinlock) {
		z_thread_sched_policy_set(&t->thread,
					 posix_policy_to_zephyr_policy(t->attr.schedpolicy));
	}
...
int pthread_setschedparam(pthread_t pthread, int policy, const struct sched_param *param)
{
	...
	SYS_SEM_LOCK(&pthread_pool_lock) {
		...
		ret = 0;
		t->attr.schedpolicy = policy;
		t->attr.priority = param->sched_priority;
		new_prio = posix_to_zephyr_priority(param->sched_priority, policy);
	}

	if (ret == 0) {
		z_thread_priority_and_policy_set(&t->thread, new_prio,
...
int pthread_getschedparam(pthread_t pthread, int *policy, struct sched_param *param)
{
	...
	SYS_SEM_LOCK(&pthread_pool_lock) {
		...
		ret = 0;
		*policy = zephyr_policy_to_posix_policy(
			z_thread_sched_policy_get(&t->thread));
		param->sched_priority = zephyr_prio_to_posix_priority(
			k_thread_priority_get(&t->thread), *policy);
	}
