...
bool move_current_to_end_of_prio_q(void)
{
...
#ifndef CONFIG_SMP
	return _kernel.ready_q.cache != curr;
#else
	struct k_thread *thread = runq_best();

	return (thread != NULL) && (z_sched_prio_cmp(curr, thread) <= 0);
#endif
}
...
void z_impl_k_thread_priority_set(k_tid_t thread, int prio)
{
	...
	z_thread_priority_and_policy_set((struct k_thread *)thread, prio,
					 z_thread_sched_policy_default(prio));
}

void z_thread_priority_and_policy_set(struct k_thread *thread, int prio, uint8_t policy)
{
	bool need_sched;

	K_SPINLOCK(&_sched_spinlock) {
		z_thread_sched_policy_set(thread, policy);
	}

	need_sched = z_thread_prio_set(thread, prio);
...
void z_impl_k_yield(void)
{
	...
	(void)move_current_to_end_of_prio_q();
	z_swap(&_sched_spinlock, key);
}

#ifdef CONFIG_TIMESLICING
void z_time_slice_yield(void)
{
	__ASSERT(arch_is_in_isr(), "");
	...
	k_spinlock_key_t key = k_spin_lock(&_sched_spinlock);
	bool switch_pending = move_current_to_end_of_prio_q();

	/*
	 * ISR-driven time slicing uses the preemptive interrupt-exit path to
	 * reach arch_switch()/arch_swap. Calling z_swap() here would be invalid
	 * on Cortex-M and many other Zephyr architectures.
	 */
	if (!switch_pending) {
