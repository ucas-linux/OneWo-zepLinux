...
static inline uint8_t z_thread_sched_policy_default(int priority)
{
	return (priority < 0) ? Z_THREAD_SCHED_FIFO : Z_THREAD_SCHED_RR;
}

static inline int z_thread_sched_policy_get(const struct k_thread *thread)
{
	return thread->base.sched_policy;
}

static inline void z_thread_sched_policy_set(struct k_thread *thread, uint8_t policy)
{
	thread->base.sched_policy = policy;
}

static inline bool z_is_thread_sched_rr(const struct k_thread *thread)
{
	int policy = z_thread_sched_policy_get(thread);

	return (policy == Z_THREAD_SCHED_RR) || (policy == Z_THREAD_SCHED_OTHER);
}
