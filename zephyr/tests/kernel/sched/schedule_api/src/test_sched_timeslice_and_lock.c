...
static int lower_prio_executed;

static void lower_prio_thread(void *p1, void *p2, void *p3)
{
	ARG_UNUSED(p1);
	ARG_UNUSED(p2);
	ARG_UNUSED(p3);

	lower_prio_executed = 1;
}
...
/**
 * @brief Verify RR time slice does not drop to a lower priority thread
 *
 * @details Create one lower-priority runnable thread while the current
 * thread is subject to time slicing. Even after multiple slice expirations,
 * the lower-priority thread must not run until a same-or-higher priority
 * alternative is ready.
 *
 * @ingroup kernel_sched_tests
 */
ZTEST(threads_scheduling, test_time_slicing_keeps_current_over_lower_prio)
{
#ifdef CONFIG_TIMESLICING
	int old_prio = k_thread_priority_get(k_current_get());
	struct k_thread lower_thread;
	k_tid_t tid;

	lower_prio_executed = 0;

	k_thread_priority_set(k_current_get(), K_PRIO_PREEMPT(0));
	k_sched_time_slice_set(200, K_PRIO_PREEMPT(0));

	tid = k_thread_create(&lower_thread, tstack, STACK_SIZE,
			      lower_prio_thread, NULL, NULL, NULL,
			      K_PRIO_PREEMPT(1), 0, K_NO_WAIT);

	k_busy_wait(500000);
	zassert_equal(lower_prio_executed, 0,
		      "timeslice incorrectly scheduled a lower-priority thread");
