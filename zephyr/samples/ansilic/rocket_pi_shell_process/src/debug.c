...
void print_stack_trace(void)
{
	...
	if (current) {
		...
		printk("Thread ID: %p\n", current);
		printk("Thread stack: %p\n", current->stack_info.start);
		printk("Thread stack size: %zu\n", current->stack_info.size);
	}

	/* Get frame pointer and print backtrace - architecture specific */
	...
	register unsigned long fp __asm__("r7");  /* ARM uses r7 as frame pointer */
	register unsigned long sp __asm__("sp");
	register unsigned long lr __asm__("lr");  /* Link register */
	...
	printk("  lr      = 0x%08lx\n", lr);
#else
	unsigned long fp = 0, sp = 0;
	printk("\nRegisters (Generic):\n");
	printk("  Stack trace not supported on this architecture\n");
#endif
