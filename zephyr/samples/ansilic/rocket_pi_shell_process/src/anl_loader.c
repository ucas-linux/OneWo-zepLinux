...
int anl_load(const char *name, const uint8_t *buf, size_t len)
{
     ...
     * Architecture-specific memory and instruction cache synchronization */
#if defined(CONFIG_RISCV)
    /* RISC-V: fence - complete all prior stores (data ordering)
...
#else
    /* Generic: just ensure compiler memory ordering */
